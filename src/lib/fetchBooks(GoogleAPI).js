...
const GOOGLE_BOOKS_API_KEY = process.env.GOOGLE_BOOKS_API_KEY || 'AIzaSyB7WrXP65MTMlbctW0U_XgO-lC553HlC1E';
const BASE_URL = 'https://www.googleapis.com/books/v1/volumes';

/*
  FETCHES TWO POPULAR BOOKS PER CATEGORY
*/
export async function fetchPopularBooks(sortBy = 'relevance') {
    ...
    try {
        for (const category of categories) {
            const response = await fetch(
                `${BASE_URL}?q=subject:${encodeURIComponent(category)}&maxResults=2&orderBy=${sortBy}&key=${GOOGLE_BOOKS_API_KEY}`,
                {
                    cache: 'force-cache'
                }
            );
        ...
        return {
            ...
            ? `Failed to fetch books: ${error.message}`: 'Network error: Please check your connection and try again.',
        };
    }
}
...
export async function fetchCategoryBooksByPage(category, page) {
    ...
    try {
        const response = await fetch(
            `${BASE_URL}?q=subject:${encodeURIComponent(category)}&maxResults=${resultsPerPage}&startIndex=${startIndex}&orderBy=newest&key=${GOOGLE_BOOKS_API_KEY}`,
            {
                cache: 'force-cache'
            }
        );
...
export async function fetchBookById(id) {
    ...
    try {
        ...
        const book = {
            url: data.volumeInfo.canonicalVolumeLink || `https://books.google.com/books?id=${id}`,
            key: data.id,
            ...
            publish_places: data.volumeInfo.publishedDate ? [data.volumeInfo.publishedDate]: [],
        ...
        const isDownloadable = data.accessInfo?.pdf?.isAvailable && !!data.accessInfo.pdf.downloadLink;
        const downloadUrl = isDownloadable ? data.accessInfo.pdf.downloadLink: null;
        ...
        return {
            ...
            ? `Book not found: Invalid or unmatched ID (${id})`: error.message.includes('API error')
            ? `Failed to fetch book: ${error.message}`: 'Network error: Please check your connection and try again.',
        };
    }
}